...
{
    // 所有可能出现的怪物
    public override IEnumerable<MonsterModel> AllPossibleMonsters => [ModelDb.Monster<PathfinderCar>()];
...
public override IReadOnlyList<string> Slots => ["first", "second", "third"];
    ...
    protected override IReadOnlyList<(MonsterModel, string?)> GenerateMonsters() => [
        (ModelDb.Monster<PathfinderCar>().ToMutable(), "third") // 如果不想指定怪物生成在哪个槽位，可以直接传null，系统会自动分配
    ];
