...

{
    ...
    {
        if (card.Owner == base.Owner && card.Type == MegaCrit.Sts2.Core.Entities.Cards.CardType.Attack && card.DynamicVars.Damage.BaseValue >= 15)
        {
            Flash();
            foreach (Creature c in base.Owner.Creature.CombatState.HittableEnemies)
            {
