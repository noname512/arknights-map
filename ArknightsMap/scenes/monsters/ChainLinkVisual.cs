...
public partial class ChainLinkVisual : Node2D
{
    ...
    [Export] public string ChainTexturePath = "res://ArknightsMap/images/monsters/chain_link.png";
    
    // 锁链视觉参数
    [Export] public float ChainWidth = 18f;
    [Export] public float CrossOffset = 8f;      // 两条链的横向间距，形成X形
    [Export] public float VerticalOffset = -25f; // 相对于怪物中心的Y轴偏移（连接到身体中部偏上）
    [Export] public int ZLayer = -5;             // 确保锁链在怪物下方渲染
    ...
    public override void _Process(double delta)
    {
        ...
        if (_source == null || _target == null || 
            !IsInstanceValid(_source) || !IsInstanceValid(_target))
        {
            QueueFree();
            return;
        }
